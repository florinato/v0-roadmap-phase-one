...
import { db, storage, auth, isConfigured, waitForFirebase } from './firebase';
...
export async function signupWithEmail(name, email, password) {
  try {
    ...
    if (!firebaseReady || !auth) {
      console.log('[v0] Firebase not ready or no auth, using mock signup');
      const mockUser = {
        uid: 'mock-' + Date.now(),
        email,
        displayName: name,
    ...
    try {
      ...
      const errorCode = firebaseError?.code || firebaseError?.message || 'unknown';
      console.error('[v0] Firebase signup failed:', errorCode);
      
      // Si Firebase falla por razones de configuración/red, usar fallback
      ...
      if (shouldFallback || errorCode.includes('network') || errorCode.includes('configuration')) {
        console.log('[v0] Firebase unavailable, falling back to mock signup');
        const mockUser = {
          uid: 'mock-' + Date.now(),
          email,
          displayName: name,
...
export async function loginWithEmail(email, password) {
  try {
    ...
    if (!firebaseReady || !auth) {
      console.log('[v0] Firebase not ready or no auth, using mock login');
      const mockUser = {
    ...
    try {
      ...
      const errorCode = firebaseError?.code || firebaseError?.message || 'unknown';
      console.error('[v0] Firebase login failed:', errorCode);
      ...
      if (shouldFallback || errorCode.includes('network') || errorCode.includes('configuration')) {
        console.log('[v0] Firebase unavailable, falling back to mock login');
        const mockUser = {
          uid: 'mock-user-' + Date.now(),
          email,
          displayName: email.split('@')[0],
...
export async function getUserTransactions(userId) {
  try {
    ...
    // Si Firestore no está disponible (índice faltante, sin permiso, etc), retornar vacío
    if (isFirestoreUnavailable(error)) {
      console.log('[v0] Firestore unavailable for transactions');
    }
