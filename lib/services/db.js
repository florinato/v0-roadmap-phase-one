...
export async function signupWithEmail(name, email, password) {
  try {
    ...
    try {
      ...
      console.error('[v0] Firebase signup failed:', firebaseError.code);
      // Si Firebase falla, usar fallback a mockData
      if (firebaseError.code === 'auth/network-request-failed' || 
          firebaseError.code === 'auth/internal-error' ||
          firebaseError.code === 'auth/configuration-not-found') {
        console.log('[v0] Firebase error, falling back to mock');
        const mockUser = {
          uid: 'mock-' + Date.now(),
          email,
          displayName: name,
...
export async function loginWithEmail(email, password) {
  try {
    ...
    try {
      ...
      console.error('[v0] Firebase login failed:', firebaseError.code);
      // Si Firebase falla, usar fallback a mockData
      if (firebaseError.code === 'auth/network-request-failed' || 
          firebaseError.code === 'auth/internal-error' ||
          firebaseError.code === 'auth/configuration-not-found') {
